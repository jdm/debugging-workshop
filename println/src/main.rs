...
struct MemoryBuffer {
  ...
  permissions: HashMap<usize, Permission>,
}
...
impl MemoryBuffer {
    ...
    fn write_memory_at(&mut self, idx: usize, val: u16) -> Result<(), ()> {
        if self.permission_at(idx) != Permission::ReadOnly {
            self.buffer[idx % self.buffer.len()] = val;
            Ok(())
        } else {
            Err(())
        }
...
/*
  implement fmt::Debug for TruncatedDebug so that it shows the first 20 characters
  of the Debug representation of the wrapped value.

  Take a look at:
  * https://doc.rust-lang.org/std/macro.format.html
  * https://doc.servo.org/alloc/string/struct.String.html#method.truncate
*/
